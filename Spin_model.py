...
class Heisenberg:
    def __init__(self, ndots : int, J : np.array, B : np.array):
        ...
        for i in range(self.l - 1):
            ...
            H_spin += - J_spin[i][i + 1] * ( self.multi_Pauli(X, i) @ self.multi_Pauli(X, i + 1) + self.multi_Pauli(Y, i) @ self.multi_Pauli(Y, i + 1) + self.multi_Pauli(Z, i) @ self.multi_Pauli(Z, i + 1) )
            H_mag += mag[i][0] * self.multi_Pauli(X, i) + mag[i][1] * self.multi_Pauli(Y, i) + mag[i][2] * self.multi_Pauli(Z, i)
        H_mag += mag[self.l-1][0] * self.multi_Pauli(X, self.l-1) + mag[self.l-1][1] * self.multi_Pauli(Y, self.l-1) + mag[self.l-1][2] * self.multi_Pauli(Z, self.l-1)
        # Hamiltonians
        self.Hspin = H_spin
        self.Hmag = H_mag
